...
[Service]
# -p (path mode), not -c (named config): a named config is ignored when the
# user also runs a Quickshell bar (its ~/.config/quickshell/shell.qml
# registers a 'default' config and suppresses all named subdirs). %h = $HOME.
ExecStart=/usr/bin/qs -p %h/.config/quickshell/music-widget
Restart=on-failure
RestartSec=2
...
# -p (path) not -c (name): named configs are suppressed when a Quickshell
# bar owns ~/.config/quickshell/shell.qml. Must match the service's path so
# IPC targets the same instance.
QS_CFG="$HOME/.config/quickshell/music-widget"
if ! qs -p "$QS_CFG" ipc call window toggle 2>/dev/null; then
    systemctl --user start music-widget.service
    for _ in 1 2 3 4 5; do
        sleep 0.4
        qs -p "$QS_CFG" ipc call window open 2>/dev/null && exit 0
    done
